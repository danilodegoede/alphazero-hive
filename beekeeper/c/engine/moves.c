...
// To sort the tile_stack elements for the action encoding:
// https://www.tutorialspoint.com/c_standard_library/c_function_qsort.htm
int cmpfunc (const void *a, const void *b) {
    // a - b > 0 betekent dat a > b
    struct tile_stack *stack_a = (struct tile_stack*)a;
    struct tile_stack *stack_b = (struct tile_stack*)b;

    // If the two tiles have the same location, we sort them on the z-level.
    if (stack_b->location - stack_a->location == 0) {
        return stack_b->z- stack_a->z;
    }

    return (stack_b->location - stack_a->location);
}
...
//    update_can_move(board, location, previous_location);
    ...
    // To remove ambiguity in the action encoding,  we base the iteration of a
    // tile on when it is encountered when we traverse the board row by row
    // from left to right.
    // Set the first 2 bits (tile iteration) to 0.
    unsigned char move_tile_no_iter = child->move.tile & (~NUMBER_MASK);
    unsigned char move_nextto_no_iter = child->move.next_to & (~NUMBER_MASK);

    // Counters for the tiles with the same color and type as the moved tile.
    int tile_counter = 0;
    int next_to_counter = 0;
    // printf("TILE MASKS: %d %d\n", child->move.tile, move_tile_no_iter);
    // printf("NEXT_TO MASKS: %d %d\n", child->move.next_to, move_nextto_no_iter);

    // We stop counting once we find the tile of interest.
    int found_tile = false;
    int found_next_to = false;
    ...
    for (int y = ly; y < hy; y++) {
        for (int x = lx; x < hx; x++) {
            unsigned char cur_tile_type = child->board->tiles[y * BOARD_SIZE + x].type;

            // Skip empty tiles
            if (cur_tile_type == EMPTY) continue;

            unsigned char cur_tile_no_iter = cur_tile_type & (~NUMBER_MASK);

            // Increase the tile_counter if we find a tile of the same color and type.
            if (move_tile_no_iter == cur_tile_no_iter && !found_tile) {
                // If we find the current tile, we increase the counter once more
                // to find the resulting iteration of the tile. After this we stop counting.
                if (child->move.tile == cur_tile_type) {
                    found_tile = true;
                }
            ...
            // Increase the next_to_counter if we find a tile of the same color and type.
            if (move_nextto_no_iter == cur_tile_no_iter && !found_next_to) {
                if (child->move.next_to == cur_tile_type) {
                    found_next_to = true;
                }
    ...
    // Edge case: We continue counting for the beetles that move on top of another tile.
    if (child->move.direction == 6) {
        struct tile_stack *stack = child->board->stack;

        qsort(stack, 4, sizeof(struct tile_stack), cmpfunc);

        for (int i = 0; i < TILE_STACK_SIZE; i++) {
            // Note that this loop content is copy-pasta.
            // Skip tile-stacks that are set inactive (location -1).
            if (stack[i].location == -1) {
                continue;
            }
            unsigned char cur_tile_type = stack[i].type;
            unsigned char cur_tile_no_iter = cur_tile_type & (~NUMBER_MASK);
    ...
    // Update the tile and next_to accordingly by setting the first two bits of the tile types.
    child->move.tile = move_tile_no_iter | (tile_counter << NUMBER_SHIFT);
    child->move.next_to = move_nextto_no_iter | (next_to_counter << NUMBER_SHIFT);

    int translate_offset = translate_board(child->board);

    child->move.location += translate_offset;
    child->move.previous_location += translate_offset;
}
