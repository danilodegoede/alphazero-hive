...
class Coach():
    ...
    def random_move_baseline(self, board):
        ...
        return np.random.choice(self.game.getActionSize(), p=valids)
    ...
    def learn(self):
        ...
        """
        # To track the iteration of the model (some iterations don't yield a new model)
        model_iteration = 1

        # Evaluate the model's initial performance (without training).
        nmcts = MCTS(self.game, self.nnet, self.args)
        arena = Arena(lambda x: self.random_move_baseline(x),
                      lambda x: np.argmax(nmcts.getActionProb(x, temp=0)), self.game)
        wins_random, wins_zero, draws = arena.playGames(50)

        log.info('ZERO/RANDOM WINS : %d / %d ; DRAWS : %d' % (wins_zero, wins_random, draws))
        ...
        for i in range(1, self.args.numIters + 1):
            ...
            if pwins + nwins == 0 or float(nwins) / (pwins + nwins) < self.args.updateThreshold:
                ...
                log.info('PITTING AGAINST RANDOM BASELINE')
                arena = Arena(lambda x: self.random_move_baseline(x),
                              lambda x: np.argmax(nmcts.getActionProb(x, temp=0)), self.game)
                wins_random, wins_zero, draws = arena.playGames(50)
